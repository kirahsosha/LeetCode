/*
 * @lc app=leetcode.cn id=3418 lang=typescript
 *
 * [3418] 机器人可以获得的最大金币数
 */
...
function maximumAmount(coins: number[][]): number {
  ...
  const solve = (rows: number, cols: number, transposed: boolean): number => {
    const negInf = Number.NEGATIVE_INFINITY;
    const dp0: number[] = new Array(cols).fill(negInf);
    const dp1: number[] = new Array(cols).fill(negInf);
    const dp2: number[] = new Array(cols).fill(negInf);
