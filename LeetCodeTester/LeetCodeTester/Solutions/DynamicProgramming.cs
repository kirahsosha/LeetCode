...
namespace LeetCodeTester.Solutions
{
    public partial class Solution
    {
        ...
        /// <param name="limit"></param>
        /// <returns></returns>
        public int NumberOfStableArrays_2(int zero, int one, int limit)
        {
            int dfs(int i, int j, int k, int limit, int[][][] memo)
            {
                if (i == 0)
        ...
        public int MaximumAmount(int[][] coins)
        {
            //i == 0 && j == 0:
            // dp[0][0] = coins[0][0]
            // dp[0][1] = max(coins[0][0], 0)
            // dp[0][2] = max(coins[0][0], 0)
            //i > 0 && j == 0:
            // dp[i][0] = dp[i - 1][0] + coins[i][0]
            // dp[i][1] = max(dp[i − 1][1] + coins[i][0], dp[i − 1][0])
            // dp[i][2] = max(dp[i − 1][2] + coins[i][0], dp[i − 1][1])
            //i == 0 && j > 0:
            // dp[i][0] = dp[i][0] + coins[i][j]
            // dp[i][1] = max(dp[i][1] + coins[i][j], dp[i][0])
            // dp[i][2] = max(dp[i][2] + coins[i][j], dp[i][1])
            //i > 0 && j > 0:
            // dp[i][0] = max(dp[i − 1][0] + coins[i][j], dp[i][0] + coins[i][j])
            // dp[i][1] = max(max(dp[i − 1][1] + coins[i][j], dp[i − 1][0]), max(dp[i][1] + coins[i][j], dp[i][0]))
            // dp[i][2] = max(max(dp[i − 1][2] + coins[i][j], dp[i − 1][1]), max(dp[i][2] + coins[i][j], dp[i][1]))
            var m = coins.Length;
